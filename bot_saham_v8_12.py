...
║                                                                  ║
║  v8.11 Fixes (Audit-Driven Hardening):                         ║
║  [Z01] WEEKEND/JAM GUARD di run() entry — guard eksplisit di   ║
║         10 baris pertama run(); return early jika di luar jam   ║
║         bursa (Senin–Jumat 08:00–17:00 WIB) sebelum koneksi    ║
║         ke Supabase/Yahoo Finance dilakukan sama sekali.        ║
║  [Z02] IHSG CACHE TERPUSAT — get_ihsg_cached() menyimpan       ║
║         hasil download ^JKSE ke modul-level dict dengan TTL    ║
║         10 menit. check_market_abnormal(), get_ihsg_regime(),  ║
║         dan check_correlation_filter() semua pakai cache ini    ║
║         — eliminasi 3 duplicate network round-trip per run.     ║
║  [Z03] VOL_DROP BUG FIX — check_position_management() menerima ║
║         parameter volumes opsional; vol_drop sekarang berbasis  ║
║         volume aktual bukan closes (price). Fallback aman ke    ║
║         price-based jika volumes tidak tersedia.               ║
║  [Z04] UNIT TESTS CORE NUMERIK — 15 test untuk fungsi          ║
║         kalkulasi kritis: calc_atr(), calc_ema(), calc_kelly_   ║
║         fraction(), calc_ev(), dan bootstrap phase detection.   ║
║         Jalankan: python bot_saham_v8_11.py --test             ║
║                                                                  ║
║  v8.12 Enhancements (Notification & Workflow):                 ║
║  [AA01] TELEGRAM 4096 FIX — tg() auto-split pesan panjang.    ║
║  [AA02] PORTFOLIO EXPOSURE di heartbeat — at-risk %, deployed  ║
║         %, open count dengan progress bar.                      ║
║  [AA03] OPEN POSITIONS SUMMARY di heartbeat — pair, entry,     ║
║         SL, TP, usia posisi.                                    ║
║  [AA04] FILTER AUDIT TOP BLOCKER di no-signal heartbeat —      ║
║         top-5 gate dengan ikon dan progress bar.               ║
║  [AA05] DAILY PNL SUMMARY — sekali jam 15:30 WIB, WIN/LOSS/   ║
║         EXPIRED + net R earned.                                 ║
║  [AA06] BOOTSTRAP PROGRESS BAR — visual [████░░] di COLD/     ║
║         EARLY/WARMING.                                          ║
║  [AA07] STREAK & EDGE HEALTH di WARMING/MATURE — loss streak,  ║
║         relaxasi aktif, strategy disabled.                      ║
║  [AA08] MODEL EVOLUTION TELEGRAM — ringkasan threshold mutasi  ║
║         setelah run_model_evolution_cycle().                    ║
║  [AA09] POSITION EXPIRY WARNING — stateless window-based,      ║
║         warn jika sisa ≤ 8 jam sebelum expired.                ║
║  [AA10] EVOLUTION TIMEZONE FIX — job terpisah via --evolution  ║
║         flag, bug _dt.now() UTC dihapus dari run().            ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
def get_filter_audit_summary() -> str:
    ...
    for i, (gate, count) in enumerate(
            sorted(_filter_audit.items(), key=lambda x: -x[1])[:5], 1):
        pct  = count / total * 100
...
def send_health_check(scanned: int, skip_vol: int, ihsg: dict, wr: dict,
    ...
    # ── [v8.12] FILTER AUDIT block (no-signal only) ───────────────────
    filter_audit_block = ""
    if no_signal:
        try:
            ...
            log(f"⚠️ [v8.12] filter audit block: {_fae}", "warn")
...
def run():
    ...
    now_wib = datetime.now(WIB)
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT SAHAM IDX v8.12 — {now_wib.strftime('%Y-%m-%d %H:%M WIB')}")
    log(f"{'='*60}")
