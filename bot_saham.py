...
def send_health_check(scanned: int, skip_vol: int, ihsg: dict, wr: dict,
    ...
    msg = (
        f"💓 <b>Bot Heartbeat — Saham IDX v8.21</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🕐 {now_str}\n"
        f"{ihsg_block}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def run():
    ...
    # Evolution sekarang: python bot_saham.py --evolution via separate GHA job
    ...
    # [B2-FIX / v8.20] Pre-loop sanity check ─────────────────────────────
    # Test 1 ticker representative SEBELUM scan 39× untuk deteksi
    # yfinance/network issue lebih awal. Mencegah loop penuh (~2 menit)
    # yang hasilnya 0 tanpa keterangan. Dipilih: ticker pertama WATCHLIST.
    _sanity_ticker = WATCHLIST[0] if WATCHLIST else None
    if _sanity_ticker:
        try:
            _sanity_data, _ = get_candles_v2(_sanity_ticker, "1d", 3)
            if _sanity_data is None:
                log(f"⚠️ [B2] Sanity FAIL: {_sanity_ticker} → None. "
                    f"yfinance mungkin down. Scan dilanjut tapi waspada.", "warn")
                tg(f"⚠️ <b>[SCAN WARNING v8.20]</b> Sanity check gagal "
                   f"(<code>{_sanity_ticker}</code> return None). "
                   f"yfinance mungkin bermasalah — pantau data_fail di run ini.")
            else:
                _s_px = float(_sanity_data[0][-1]) if _sanity_data[0] is not None else 0
                log(f"  ✅ [B2] Sanity OK: {_sanity_ticker} = Rp{_s_px:,.0f}")
        except Exception as _sanity_e:
            log(f"⚠️ [B2] Sanity EXCEPTION: {type(_sanity_e).__name__}: {_sanity_e} "
                f"— network/yfinance bermasalah?", "warn")
            tg(f"⚠️ <b>[SCAN WARNING v8.20]</b> Sanity check exception: "
               f"<code>{type(_sanity_e).__name__}</code>. "
               f"Cek koneksi / yfinance status.")
    ...
    for ticker in WATCHLIST:
        try:
            ...
            # [B1-FIX / v8.20] Outer except WAJIB increment data_fail.
            # Bug lama: exception ditelan diam-diam → data_fail = 0 meski
            # semua ticker gagal → cascade check blind → SCAN MASUK = 0
            # tanpa penjelasan apapun. (Root cause screenshot 04/05/2026)
            data_fail += 1
            import traceback as _tb_outer
            _exc_type  = type(e).__name__
            _exc_frame = _tb_outer.extract_tb(e.__traceback__)[-1] \
                         if e.__traceback__ else None
            _loc_str   = (f" [{_exc_frame.name}:{_exc_frame.lineno}]"
                          if _exc_frame else "")
            log(f"⚠️ [B1] [{ticker}] {_exc_type}: {e}{_loc_str}", "warn")
            continue
    ...
    # [B3-FIX / v8.20] Mass-exception detector ──────────────────────────
    # Berlapis di atas cascade check — menangkap kasus di mana data_fail
    # tinggi bukan dari return-None path tapi dari outer-except path (B1).
    # Threshold 70%: hampir pasti bukan masalah individual ticker.
    if fail_ratio >= 0.70:
        log(
            f"🔴 [B3] MASS EXCEPTION: {data_fail}/{total_attempted} "
            f"ticker gagal ({fail_ratio:.0%}). "
            f"Cek log untuk exception type per ticker.",
            "error"
        )
        tg(
            f"🔴 <b>[SCAN ABORT — MASS EXCEPTION v8.20]</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Gagal: <b>{data_fail}/{total_attempted}</b> ticker ({fail_ratio:.0%})\n"
            f"Tipe: outer exception (exception di loop body)\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Kemungkinan penyebab:\n"
            f"  • yfinance timeout / rate-limit\n"
            f"  • Network issue di GitHub Actions runner\n"
            f"  • Perubahan API yfinance\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"<i>Cek log GHA untuk exception type spesifik [B1].\n"
            f"Scan berikutnya dalam 4 jam.</i>"
        )
        return
    ...
    tg(f"🔍 <b>Scan Selesai — Signal Bot Saham IDX v8.21</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       ...
       f"{wr_line}"
       f"{exposure_line}\n"
       f"🧠 <i>v8.21 — Data Collection Mode aktif.</i>\n"
       f"<i>Scan berikutnya dalam 4 jam.</i>")
...
def _fetch_yfinance_flagged(
    ...
    try:
        ...
        # [v8.21 FIX] yfinance ≥0.2.50 kadang return MultiIndex columns
        # bahkan untuk single ticker (e.g. ('Close','BBCA.JK') bukan 'Close').
        # df["Close"] pada MultiIndex → returns DataFrame, bukan Series
        # → .astype(float) gagal dengan TypeError.
        # Fix: flatten MultiIndex ke level 0, sama seperti get_candles().
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
...
if __name__ == "__main__":
    ...
    if "--backtest" in _args:
        ...
        # [Z04] Unit tests core numerik — jalankan: python bot_saham.py --test
