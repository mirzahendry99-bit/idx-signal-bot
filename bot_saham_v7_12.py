...
def detect_market_phase(closes, highs, lows, volumes) -> dict:
    ...
    # [v7.11 FIX] Guard semua slice agar tidak crash jika n < 30
    # Gunakan max available candles dengan batas minimum yang aman
    w_recent = min(5, n)
    w_prev   = min(15, n)
    w_range  = min(10, n)
    w_range_prev = min(30, n) - w_range  # pastikan ada candle untuk prev range
...
def send_health_check(scanned: int, skip_vol: int, ihsg: dict, wr: dict,
    ...
    msg = (
        f"💓 <b>Bot Heartbeat — Saham IDX v7.12</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🕐 {now_str}\n"
...
def check_intraday(ticker: str, price: float, ihsg: dict, side: str = "BUY") -> dict | None:
    ...
    # Hard block: jika tanda OUTFLOW kuat dan confidence tinggi, jangan counter
    if (side == "BUY" and foreign_flow["trend"] == "OUTFLOW"
            and foreign_flow["confidence"] >= FOREIGN_FLOW_BLOCK_CONF):
        log(f"  ⚠️ {ticker} [1H BUY]: Net foreign proxy OUTFLOW (score={foreign_flow['score']:.2f}, conf={foreign_flow['confidence']:.0%}) — skip")
        return None
    ...
    if side == "BUY":
        ...
        score = score_signal("BUY", price, closes, highs, lows, volumes,
                             ...
                             ema20, ema50, vwap, mkt["regime"], weights=merged_w, opens=opens)
        ...
        # [v6.0+v7.9] Setup Ranking — boost skor dan adjust EV threshold per setup quality
        ...
        # [v7.9 FIX] Tier di-assign SETELAH semua bonus (sniper + setup rank)
        # agar label tier mencerminkan score final yang sesungguhnya
    ...
    # [v7.9 FIX] Tambahkan ticker agar win_prob engine bisa pakai cluster WR aktual dari Supabase
...
def check_swing(ticker: str, price: float, ihsg: dict, side: str = "BUY") -> dict | None:
    ...
    # ── [v7.9] Net Foreign Flow Proxy ────────────────────
    # Untuk swing (1D), lookback lebih panjang = lebih akurat
    foreign_flow = get_net_foreign_proxy(closes, highs, lows, volumes, lookback=15)
    if (side == "BUY" and foreign_flow["trend"] == "OUTFLOW"
            and foreign_flow["confidence"] >= FOREIGN_FLOW_BLOCK_CONF):
        log(f"  ⚠️ {ticker} [1D BUY]: Net foreign proxy OUTFLOW (score={foreign_flow['score']:.2f}, conf={foreign_flow['confidence']:.0%}) — skip")
        return None
    ...
    if side == "BUY":
        ...
        score = score_signal("BUY", price, closes, highs, lows, volumes,
                             structure, liq, ob, rsi, macd, msig,
                             ema50, ema200, vwap, mkt["regime"], weights=merged_w, opens=opens)
    ...
    # [v7.9 FIX] Tambahkan ticker agar pakai cluster WR aktual
    win_prob = calc_win_probability(score, max_positive, mkt["regime"],
                                    phase, trigger["strength"], bias_aligned,
                                    ticker=ticker)
    ev = calc_expected_value(win_prob, rr)
...
def send_signal(sig: dict):
    ...
    msg = (
        ...
        f"⚠️ Data delay ~{DATA_DELAY_MINUTES} menit (yfinance)\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def run():
    ...
    now_wib = datetime.now(WIB)
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT SAHAM IDX v7.12 — {now_wib.strftime('%Y-%m-%d %H:%M WIB')}")
    log(f"{'='*60}")
    ...
    allow_sell = True
    # [v7.2 — FIX Masalah 5] IHSG drop ringan tapi counter-trend sectors tetap bisa BUY
    # (dihandle di is_sector_blocked per-ticker — allow_buy tetap True agar scan berjalan,
    #  counter-trend exemption dicek saat is_sector_blocked dipanggil per ticker)
    if ihsg["block_buy"] and not ihsg["halt"]:
        ...
        allow_buy = True  # biarkan scan jalan, is_sector_blocked yang filter per sektor
    ...
    for ticker in WATCHLIST:
        try:
            ...
            data_1d = get_candles(ticker, "1d", 25)  # FIX 6C: butuh min 25 candle untuk 20-day avg
            if data_1d is None:
                data_fail += 1
                continue
    ...
    # FIX 6A: Data health sanity check
    # Jika > 40% ticker gagal ambil data → kemungkinan besar outage yfinance/IDX maintenance
    # Abort dengan CRITICAL alert agar bisa dibedakan dari "no signal karena kondisi pasar"
    ...
    sent = 0
    cumulative_new_risk_pct = 0.0  # FIX 2B: track exposure dari signal baru di cycle ini
    for sig in signals:
        if sent >= signals_cap: break   # [v7.0] portfolio-aware cap

        # FIX 2B: pre-send exposure check — pastikan penambahan signal ini
        # tidak membuat total exposure (existing + baru) melewati circuit breaker
        ...
        if projected_total > KS_DRAWDOWN_PCT_MAX:
            ...
            break   # hentikan dispatch lebih lanjut dalam cycle ini
        ...
        sent += 1
        time.sleep(1.2)  # FIX 4B: naikkan delay 0.5 → 1.2s untuk hindari Telegram 429
    ...
    tg(f"🔍 <b>Scan Selesai — Signal Bot Saham IDX v7.12</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       ...
       f"{wr_line}"
       f"{exposure_line}\n"
       f"🧠 <i>v7.12 — Production-Ready aktif</i>\n"
       f"<i>Scan berikutnya dalam 4 jam.</i>")
